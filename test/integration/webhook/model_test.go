...
var _ = ginkgo.Describe("model default and validation", func() {
	...
	ginkgo.DescribeTable("Defaulting test",
		...
		ginkgo.Entry("apply model family name", &testDefaultingCase{
			...
			wantModel: func() *core.Model {
				return wrapper.MakeModel("llama3-8b").ModelSourceWithModelID("meta-llama/Meta-Llama-3-8B").ModelSourceWithModelHub("Huggingface").FamilyName("llama3").Label(core.ModelFamilyNameLabelKey, "llama3").Obj()
			},
		}),
		ginkgo.Entry("apply modelscope model hub name", &testDefaultingCase{
			...
			wantModel: func() *core.Model {
				return wrapper.MakeModel("llama3-8b").ModelSourceWithModelID("LLM-Research/Meta-Llama-3-8B").ModelSourceWithModelHub("ModelScope").FamilyName("llama3").Label(core.ModelFamilyNameLabelKey, "llama3").Obj()
			},
		}),
	)
	...
	ginkgo.DescribeTable("test validating",
		...
		// 	model: func() *core.Model {
		// 		return wrapper.MakeModel("llama3-8b").FamilyName("llama3").ModelSourceWithURI("image://meta-llama-3-8B").Obj()
		// 	},
		// 	failed: false,
		// }),
